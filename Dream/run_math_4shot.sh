...
OUTPUT_ROOT_DEFAULT="${SCRIPT_DIR}/math_sweeps/${timestamp}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$OUTPUT_ROOT_DEFAULT}"

usage() {
  cat <<'EOF'
Usage:
  bash run_math_4shot.sh [options]
...
for block_length in "${BLOCK_LENGTHS[@]}"; do
  for mode_name in fast_dllm_dual_cache; do
    ...
    model_args="pretrained=${MODEL_PATH},max_new_tokens=${MAX_NEW_TOKENS},diffusion_steps=${STEPS},add_bos_token=true,alg=entropy,block_length=${block_length},use_cache=true,dual_cache=true"
  ...
  for focus_layer in "${FOCUS_LAYERS[@]}"; do
    for focus_topk in "${FOCUS_TOPKS[@]}"; do
      ...
      model_args="pretrained=${MODEL_PATH},max_new_tokens=${MAX_NEW_TOKENS},diffusion_steps=${STEPS},add_bos_token=true,alg=entropy,block_length=${block_length},use_cache=true,dual_cache=true,focus_decode=true,focus_layer=${focus_layer},focus_topk=${focus_topk}"
      if [[ -n "${EXTRA_MODEL_ARGS}" ]]; then
        model_args="${model_args},${EXTRA_MODEL_ARGS}"
      fi
